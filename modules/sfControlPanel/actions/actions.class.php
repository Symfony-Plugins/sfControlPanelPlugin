...
class sfControlPanelActions extends sfActions
{
  ...
  {
    $this->setLayout(sfLoader::getTemplateDir('sfControlPanel', 'layout.php').'/layout');
    $controllers    = sfFinder::type('file')->relative()->ignore_version_control()->maxdepth(1)->name('*.php')->in(sfConfig::get('sf_web_dir'));
    ...
    foreach ($apps as $app)
    {
      ...
      {
        $contents = file_get_contents(sfConfig::get('sf_web_dir').'/'.$controller);
  ...
  public function executeIndex()
  {
    ...
    if($files = sfFinder::type('file')->name("*schema.*")->maxdepth(3)->relative()->prune(sfConfig::get('sf_web_dir'))->prune('lib')->prune('data')->ignore_version_control()->in(SF_ROOT_DIR))
    {
